...
def create_app():
    ...
    def save_generation(project_id, chunk_id):
        ...
        if text:
            gen = ChunkGeneration(project_id=proj.id, chunk_id=ch.id, generated_text=text, sources=None)
            db.session.add(gen)
    ...
    def assemble_docx(project_id):
        ...
        return send_file(out_docx, as_attachment=True, download_name=os.path.basename(out_docx))
